...
class BottomDrawerContainer(context: Context?, attrs: AttributeSet? = null)
    ...
    private fun handleTouchEvent(ev: MotionEvent?): Boolean {
        ...
        if (handle) {
            ...
            if (isOpened()) {
                ...
                if (DEBUG) Log.i(TAG, "handleTouchEvent state=closed, mMoveCount=$mMoveCount, yDiff=$yDiff, TOUCH_SLOP=$TOUCH_SLOP")
    ...
    fun openDrawer() {
        ...
        val animator = ValueAnimator.ofFloat(mTarget!!.y, (mMeasureHeight - mTargetHeight).toFloat())
